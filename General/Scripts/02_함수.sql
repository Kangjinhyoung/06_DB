...
SELECT 'HELLO WORLD' 문자, LENGTH('HELLO WORLD') "문자 길이"
FROM DUAL; -- 문자 HELLO WORLD , 문자 길이 11
...
SELECT 'AABAACAABBAA' 문자열, INSTR('AABAACAABBAA', 'B', 5) 위치
FROM DUAL; -- 9
...
-- SUBSTR() + INSTR() 같이 사용
-- EMPLOYEE 테이블에서 사원명, 이메일 아이디 (@ 앞에까지 문자열)을
-- 이메일 아이디 오름차순으로 조회
...
/* MONTH_BETWEEN이 지정된 두 날짜 사이의 차를 계산할때 훨씬 좋다(더 정확)
 * -> 달 마다 길이(28, 29, 30, 31)가 다 다르기 때문에 직접 계산하면 오차가 있을 수 있는데
 * 		MONTH_BETWEEN을 이용하면 이런 부분까지 모두 자동으로 적용되어 계산된다!
 * */

-- 모든 사원의 이름, 입사일, N년차  조회
SELECT EMP_NAME, HIRE_DATE, CEIL(MONTHS_BETWEEN(CURRENT_DATE, HIRE_DATE) / 12) || '년차' 년차
FROM EMPLOYEE;

------------------------------------------------------------------------------------------------

-- ADD_MONTH(날짜, 숫자) : 날짜를 순자만큼의 개월 수를 더하여 반환
SELECT 
	CURRENT_DATE, -- 8/16
...
-- 다음달 1일, 다음달 말일, 다음달 일수 조회하기
SELECT 
	LAST_DAY(CURRENT_DATE) + 1 "다음달 1일",
	LAST_DAY(ADD_MONTHS(CURRENT_DATE, 1)) "다음달 말일",
	LAST_DAY(ADD_MONTHS(CURRENT_DATE, 1)) - LAST_DAY(CURRENT_DATE) "다음달 일수"
FROM DUAL;
...
--> 지정된 포맷의 칸 수가 변환하려는 숫자 자릴수보다 적은 경우 모든 숫자가 #으로 변환되서 출력
