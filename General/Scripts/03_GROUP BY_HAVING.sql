...
/* WHERE와 HAVING의 차이
 * 
 * WHERE  : FROM절에서 지정된 테이블 전체 행 중 조건에 맞는 행만 추려내는 구문
 * 
 * HAVING : GROUP BY를 통해 만들어진 그룹 중 어던 조건에 맞는 그룹만 골라내는 구문
 * 
 * */
...
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;
--> 개입 급여가 350만 이상인 사람들의 부셔별 평균(잘못된 SQL)
...
ORDER BY DEPT_CODE ASC;
--> EMPLOYEE 테이블에서 DEPT_CODE 기준으로 만들어진 그룹 중 그룹별 급여 평균이 350만 이상인 그룹만 조회
...
WHERE BONUS IS NOT NULL
GROUP BY JOB_CODE
ORDER BY JOB_CODE ASC; -- 4행 조회(0명은 조회결과 포함 X)
