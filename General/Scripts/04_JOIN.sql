...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);  -- 하동운, 이오리 제외
...
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+);
-- DEPT_CODE의 값이 DEPT_ID와 일치하지 않아도 결과에 포함
...
-- INNER JOIN 결과에 포함되지 못했던 EMPLOYEE 테이블의 하동운, 이오리
-- DEPARTMENT 테이블의 D3, D4, D7 모두를 JOIN 결과에 포함시켜라 == LEFT UNION(합집합) RIGHT와 같은 결과
...
-- 사원의 급여가 SAL_LEVEL에 작성된 최소 ~ 최대 범위의 급여가 맞을때만 결과에 포함하겠다는 JOIN
...
-- ANSI 표준
SELECT EMP.EMP_NAME 사원명, NVL(EMP.MANAGER_ID, '없음') 사수번호, NVL(MGR.EMP_NAME, '없음') 사수명
FROM EMPLOYEE EMP
