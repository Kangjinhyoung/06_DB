...
SELECT EMP_ID 사번, EMP_NAME 이름, DEPT_TITLE 부서명, JOB_NAME 직급명, SALARY 급여
FROM EMPLOYEE
...
-- 부서별 인원수가 가정 적은 부서의 부서명, 인원 수 조회
...
-- 인라인뷰 : FROM절에 사용된 서브쿼리  --> 서브 쿼리 결과가 테이블처럼 인식
SELECT 이름, 급여
FROM (SELECT EMP_NAME 이름, SALARY 급여 FROM EMPLOYEE)
WHERE 급여 >= 4000000;
-- 메인쿼리 해석 1순위인 FROM절에 작성된
--> 서브쿼리 결과 컬럼명이 "급여"이기 때문에 메인쿼리 해석 2순위인 WHERE 
--> 3순위인 SELECT절에서도 똑같이 "급여"라고 컬럼명을 작성해야 한다.
...
-- 1) 사수에 해당하는 사원 번호 조회 --> MANAGER_ID 컬럼에 작성되있는 사번을 가진 사원 == 사수
SELECT DISTINCT MANAGER_ID
FROM EMPLOYEE
WHERE MANAGER_ID IS NOT NULL;

-- 2) 직원의 사번, 이름, 부서명, 직급 조회
SELECT EMP_ID, EMP_NAME, NVL(DEPT_CODE, '없음') 부서명, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);

-- 3) 사수에 해당하는 직원에 대한 정보 추출 조회 (이때, 구분은 '사수'로)
SELECT EMP_ID, EMP_NAME, NVL(DEPT_CODE, '없음') 부서명, JOB_NAME, '사수' AS "구분"
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_ID IN (
...
SELECT EMP_ID, EMP_NAME, NVL(DEPT_CODE, '없음') 부서명, JOB_NAME, '사원' AS "구분"
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_ID NOT IN (
...
--> 여러 컬럼을 묶어서 한 번에 비교해 모두 같은 행만 조회 결과에 포함시킴
...
WHERE EXISTS ( -- 서브쿼리 조회 결과가 있으면 TRUE == 해당 행을 결과에 포함
	SELECT '있음' -- 서브쿼리에서 조회되는 컬럼 값은 중요한게 아닙! 조회되는 데이터가 있는지 없는지가 중요!!
	FROM EMPLOYEE SUB
...
FROM EMPLOYEE "MAIN" -- 메인 쿼리 별칭이 MAIN
WHERE SALARY > (
	SELECT AVG(SALARY)
	FROM EMPLOYEE "SUB" -- 서브 쿼리 별칭이 SUB
...
JOIN JOB USING (JOB_CODE)
WHERE HIRE_DATE = (
	-- 메인쿼리 1행을 해석했을때 조회되는 행 중에서 DEPT_CODE 값을 얻어와 서브쿼리에서 
 ...
 * -> SELECT는 가능하지만 INSERT, UPDATE, DELETE는 불가능하다.
 * */
...
WHERE ROWNUM <= 5
ORDER BY SALARY DESC;
--> 조회 결과가 잘못됨! EMPLOYEE 테이블에서 5명만 조회, 정렬된 결과
...
FROM 
	(SELECT EMP_NAME, SALARY
	 FROM EMPLOYEE
	 ORDER BY SALARY DESC)
	 --> 서브쿼리 결과 (23행 2열)를 메인쿼리의 테이블로 인식됨
...
-- 1) RANK() OVER : OVER() 괄호에 작성된 정렬 기준대로 정렬 후 순위 지정함 
...
-- 2) DENS_RANK() OVER : 공동 순위 지정 후 순위 건너뛰기를 하지 않음
SELECT 
