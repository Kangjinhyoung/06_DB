...
-- ORA-01441 : 일부 값이 너무 커서 길이를 줄일 수 없음
SELECT
	DEPT_TITLE,
	LENGTH(DEPT_TITLE),
	LENGTHB(DEPT_TITLE)
...
ALTER TABLE DEPT_COPY
ADD (DEPT_TITLE VARCHAR2(30)); -- DEPT_TITLE 추가됨
...
-- TB1(부모) 테이블 삭제
DROP TABLE TB1;
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다 == PK컬럼이 다른 테이블에 참조를 당하고 있다
-- 해결방법1) 참조하는 자식 테이블을 삭제하거나 FK 제약조건을 삭제
-- 해결방법2) CASCADE CONSTRAINTS 옵션 사용하기
