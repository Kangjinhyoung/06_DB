...
-- ORA-00955: 기존의 객체가 이름을 사용하고 있습니다. -> OR REPLACE 옵션을 사용해서 해결
...
-- VIEW에 INSERT를 수행 했지만 VIEW를 만들때 사용한 원본 테이블에 값이 INSERT됨을 확인!!
--> 하지만 모든 컬럼값이 INSERT 된것이 아니라 VIEW를 생성할 때 사용된 
--  컬럼에만 값이 INSERT 되어 반대로 사용되지 않은 컬럼에는 NULL이 들어감
--> NULL은 DB의 무결성을 약하게 만드는 주요 원인!(가능하면 의도되지 않은 NULL은 존재하지 않게 하자)
